...
namespace App\Modules\Order\Infrastructure\Jobs;
...
class SyncOrdersJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        $logger->log($this->organizationId, 'Started orders synchronization dispatch to Go Service', 'sync_start');
        ...
        foreach ($connections as $connection) {
            ...
            try {
                ...
                $logger->log($this->organizationId, 'Orders sync task dispatched for '.$connection->marketplace->label(), 'sync_dispatched', [
                    'marketplace' => $marketplaceValue,
                ]);

                Prometheus::addCounter('sync_duration_seconds_sum')->inc($duration, [$marketplaceValue, 'dispatch_orders']);
                Prometheus::addCounter('sync_duration_seconds_count')->inc(1, [$marketplaceValue, 'dispatch_orders']);
                Prometheus::addCounter('sync_dispatched_total')->inc(1, [$marketplaceValue, 'orders']);
            } catch (Throwable $e) {
                Prometheus::addCounter('sync_errors_total')->inc(1, [$marketplaceValue, 'dispatch_error']);

                $logger->log($this->organizationId, sprintf('Failed to dispatch orders sync for %s: ', $marketplaceValue).$e->getMessage(), 'sync_error', [
                    'marketplace' => $marketplaceValue,
                ]);
            }
        }
    ...
    public function failed(Throwable $exception): void
    {
        resolve(ActivityLoggerInterface::class)->log(
            $this->organizationId,
            'Orders synchronization dispatch failed: '.$exception->getMessage(),
            'sync_error'
        );
